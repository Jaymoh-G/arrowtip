...
/* SWIPER SLIDER JS
	********************************************/
	rgen.swiper_slider = function (obj) {
		...
		var config = {
			fullsize: rgen.getvar ? rgen.getvar($(obj).attr('data-fullsize'), false, 'b') : false
		};
		...
		var swiper = new Swiper(obj, {
			...
			autoplay: $(obj).attr('data-autoplay') ? ($(obj).attr('data-autoplay') === "false" ? undefined : { delay: $(obj).attr('data-autoplay-delay') || 5000 }) : { delay: 5000 },
			direction: $(obj).attr('data-direction') || "horizontal",
			effect: $(obj).attr('data-slide-effect') || "slide",
			speed: parseInt($(obj).attr('data-slide-speed'), 10) || 600,
			loop: isNovi ? false : $(obj).attr('data-loop') !== "false",
			simulateTouch: $(obj).attr('data-simulate-touch') && !isNovi ? $(obj).attr('data-simulate-touch') === "true" : true,
		});
	}
	...
	********************************************/
	...
	if ($o.swiperwidget) {
		for (var i = 0; i < $o.swiperwidget.length; i++) {
			var $container = $($o.swiperwidget[i]).find('.swiper-container');
			var swiObj = 'swiper' + i;
			if (!$container.attr("id")) $container.attr("id", swiObj);
			$container.addClass(swiObj);
			rgen.swiper_slider("#" + $container.attr("id"));
		}
	}
	// Swiper gallery mode
	if ($o.swipergallery) {
